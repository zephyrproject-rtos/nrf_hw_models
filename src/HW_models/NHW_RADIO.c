...
static void nhw_radio_device_address_match(uint8_t rx_data[]);
 ...
 * And store in next_check_time the next time we need to recheck
 */
static void update_abort_struct(p2G4_abort_t *abort, bs_time_t *next_check_time){
  ...
  *next_check_time = nsi_hws_get_next_event_time();
  abort->recheck_time = hwll_phy_time_from_dev(*next_check_time);
...
static void nhw_radio_device_address_match(uint8_t rx_data[]) {
  ...
  for (int i = 0 ; i < 8; i++) {
    ...
    if (TxAdd != ((rx_data[0] >> 6) & 1) ) {
      continue;
    }

    nomatch = (*(uint32_t *)(rx_data + 2) != NRF_RADIO_regs.DAB[i]);
    uint32_t DAP = NRF_RADIO_regs.DAP[i] & UINT16_MAX;
    nomatch |= (*(uint16_t *)(rx_data + 6) != DAP);
