...
static bs_time_t overflow_timer_sub_us[N_RTC] = {TIME_NEVER}; //when the timer will overflow (in sub-microsecond units)
static bs_time_t RTC_counter_startT_sub_us[N_RTC] = {TIME_NEVER}; //Time when the counter was "started" (really the time that would correspond to COUNTER = 0)
static bs_time_t RTC_counter_startT_negative_sub_us[N_RTC] = {0};

static uint32_t counter[N_RTC] = {0}; //Internal counter value when the counter was stopped

static bs_time_t first_lf_tick_time_sub_us = 0;
...
static bs_time_t get_hw_time_sub_us()
{
...
static bs_time_t get_last_lf_tick_time_sub_us() {
  bs_time_t now_sub_us = get_hw_time_sub_us();

  uint64_t lf_ticks = (now_sub_us - first_lf_tick_time_sub_us) / LF_CLOCK_PERIOD; //floor()
  bs_time_t last_tick_time_sub_us = lf_ticks * LF_CLOCK_PERIOD;
 ...
 */
static uint64_t time_sub_us_to_counter(bs_time_t delta_sub_us, int rtc) {
  uint64_t ticks;
 ...
 */
static bs_time_t counter_to_time_sub_us(uint64_t counter, int rtc) {
  bs_time_t Elapsed;
 ...
 */
static bs_time_t time_of_1_counter_wrap_sub_us(int rtc) {
  return counter_to_time_sub_us((uint64_t)RTC_COUNTER_MASK + 1, rtc);
}

static bs_time_t get_counter_match_time(uint64_t counter_match, int rtc, bs_time_t* next_match_sub_us)
{
  ...
  *next_match_sub_us = TIME_NEVER;
  ...
  if (RTC_Running[rtc] == true) {
    bs_time_t now_sub_us = get_hw_time_sub_us();
    bs_time_t counter_match_sub_us = counter_to_time_sub_us(counter_match, rtc);
...
static void update_cc_timer(int rtc, int cc) {
  bs_time_t match_sub_us; // Only to comply to the interface
 ...
 * to an appropriate value, so that the time ellapsed from the counter start corresponds the given
...
static void set_counter_to(uint64_t counter_val, int rtc)
{
  ...
  bs_time_t counter_val_sub_us = counter_to_time_sub_us(counter_val, rtc);

  // All the functions which use this reset the <PRESC>, so it is like the counter was set
  // on the last LF clock tick
  bs_time_t last_lf_tick_sub_us = get_last_lf_tick_time_sub_us();
...
static void handle_overflow_event(int rtc)
{
  ...
  if ( overflow_timer[rtc] ==  Timer_RTC ) //Overflow occured now
  {
    ...
    bs_time_t current_overflow_event_sub_us = overflow_timer_sub_us[rtc];
 ...
 */
void nrf_rtc_TASKS_TRIGGER_OVERFLOW(int rtc) {
...
void nrf_rtc0_TASKS_TRIGGER_OVERFLOW()  { nrf_rtc_TASKS_TRIGGER_OVERFLOW(0); }
...
void nrf_rtc1_TASKS_TRIGGER_OVERFLOW()  { nrf_rtc_TASKS_TRIGGER_OVERFLOW(1); }
...
void nrf_rtc2_TASKS_TRIGGER_OVERFLOW()  { nrf_rtc_TASKS_TRIGGER_OVERFLOW(2); }
...
void nrf_rtc_regw_sideeffect_TASKS_TRIGGER_OVERFLOW(int i) {
  NRF_RTC_Type *RTC_regs = &NRF_RTC_regs[i];
  if ( RTC_regs->TASKS_TRIGOVRFLW ){
    NRF_RTC_regs[i].TASKS_TRIGOVRFLW = 0;
    nrf_rtc_TASKS_TRIGGER_OVERFLOW(i);
  }
}
...
void nrf_rtc_regw_sideeffects(int i){
  ...
  nrf_rtc_regw_sideeffect_TASKS_TRIGGER_OVERFLOW(i);
